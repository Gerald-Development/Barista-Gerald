...
public class MathCommand extends AbstractCommand {
    ...
    private static class ArithmeticToken {
        ...
        private static class ArithmeticOperator{
            ...
            public List<ArithmeticToken> apply(List<ArithmeticToken> tokens){
                ...
                int[] opIndices = IntStream.range(0, newTokens.size())
                        ...
                        .toArray();
                for(int i:opIndices){
                    if(i == newTokens.size() - 1) throw new ArithmeticException(String.format("Expression ended with a '%c' unexpectedly",this.operator));
                    if(i == 0) throw new ArithmeticException(String.format("Expression cannot start with '%c'",this.operator));
                    if(newTokens.get(i+1).type != TokenType.VALUE || newTokens.get(i-1).type != TokenType.VALUE) throw new ArithmeticException(String.format("'%c' not used like an operator",this.operator));
        ...
        public ArithmeticToken evalToken(long creation) throws RuntimeErrorException, ArithmeticException{
            ...
            if(tokens.size()==1 && tokens.get(0).type == TokenType.VALUE) return tokens.get(0);
                ...
                throw new RuntimeErrorException(new Error("AAAAAAAAAAAAAAAAAAAAAAAAAAAH how did we get more than 2 tokens or a token that isn't a value??????"));
            }
        ...
        public static List<ArithmeticToken> tokenize(String expression, long creation) throws TimeoutException, ArithmeticException{
            ...
            for(int parsePoint = 0, parenLevel = 0, tempSum = -1, tempDecimal = -1, prevParenOpen=-1;
                parsePoint<expression.length(); parsePoint++){
                ...
                if(parsedChar == ')'){
                    ...
                    else if(parenLevel < 0) throw new ArithmeticException("You cannot have a close paren befoere its opening paren");
                }
                    ...
                    switch(parsedChar){
                        ...
                        default:
                            ...
                            if(tempDecimal == -1){
                                ...
                                if(parsePoint == expression.length() - 1){
                                    children.add(new ArithmeticToken(tempSum));
                                }
                            }
                            else {
                    ...
                    if(children.size() > 0 && children.get(children.size()-1).type == TokenType.TIMEOUT) throw new TimeoutException("Child timed out");
                }
        ...
        public static List<ArithmeticToken> parseTokens(List<ArithmeticToken> tokens){
            List<ArithmeticToken> newTokens = copyTokenList(tokens);
            ...
            ArithmeticOperator
                    exponentOperator = new ArithmeticOperator('^', (n1, n2) -> Math.pow(n1, n2)),
                    multiplicationOperator = new ArithmeticOperator('*', (n1, n2) -> n1 * n2),
                    divisionOperator = new ArithmeticOperator('/', (n1, n2) -> n1 / n2),
                    minusOperator = new ArithmeticOperator('-', (n1, n2) -> n1 - n2),
                    additionOperator = new ArithmeticOperator('+', (n1, n2) -> n1 + n2);
        ...
        public static List<ArithmeticToken> copyTokenList(List<ArithmeticToken> tokens){
            ...
            for(ArithmeticToken token:tokens){
                switch(token.type){
                    case OPERATOR: newTokens.add(new ArithmeticToken(token.operatorMaybe)); break;
                    case VALUE: newTokens.add(new ArithmeticToken(token.valueMaybe)); break;
                    case EXPRESSION: newTokens.add(new ArithmeticToken(token.expressionMaybe)); break;
                    case TIMEOUT: newTokens.add(new ArithmeticToken()); break;
                }
            }
    ...
    public void executeInternal(Message message, List<String> args) {
        ...
        try{
            ...
            message.getChannel().sendMessage(
                    String.format("Arithmatic error %s",
                            e.getMessage())).queue();
        }
