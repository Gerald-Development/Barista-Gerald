...
public class ImpersonateCommand extends AbstractCommand {
    ...
    private void sendWebhookMessage(CommandContext context, List<String> args, Member memberToBeImpersonated) {
        ...
        Webhook impersonateHook = webhookService.getOrCreateWebhook((TextChannel) context.getChannel(), "BGImpersonate", context.getJDA().getSelfUser().getId());
        webhookService.postMessage(messageToBeSent.toString(), null, memberToBeImpersonated.getUser().getAvatarUrl(), memberToBeImpersonated.getUser().getName(), impersonateHook);
