            ...
            for (int parsePoint = 0, parenLevel = 0, tempSum = -1, tempDecimal = -1,
                ...
                if (parsedChar == ')') {
                    ...
                    if (parenLevel == 0) {
                        ...
                        throw new ArithmeticException("You cannot have a close paren before its opening paren");
                    implicitMul = false;
                } else if (parenLevel == 0) {
                        ...
                        default:
                            ...
                            if (tempDecimal == -1) {
                                ...
                                if (parsePoint == expression.length() - 1)
                                    children.add(constructDecimal(tempSum, tempDecimal, leadingZeroes));
                            }
                    }
        ...
        private static class ArithmeticOperator {
            ...
            public List<ArithmeticToken> apply(List<ArithmeticToken> tokens) throws ArithmeticException {
                ...
                if (inverse.isPresent()) {
                    if (!inverseAction.isPresent())
                        throw new Error(String.format("Uh oh somebody fucked with the code related to %s where they weren't supposed to", this.getClass().getCanonicalName()));
